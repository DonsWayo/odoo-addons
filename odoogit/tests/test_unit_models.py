...
class TestRepositoryUnit(OdooGitCommon):
    ...
    def test_manager_bypasses(self):
        mgr = self._create_user('mgr', groups=['odoogit.group_git_manager'])
        r = self._repo(visibility='private')
...
@tagged('unit', 'post_install', '-at_install')
class TestBranchUnit(OdooGitCommon):
...
@tagged('unit', 'post_install', '-at_install')
class TestTokenUnit(OdooGitCommon):
...
@tagged('unit', 'post_install', '-at_install')
class TestPullRequestUnit(OdooGitCommon):
