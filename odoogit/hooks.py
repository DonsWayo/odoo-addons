...
DEFAULT_PARAMS = {
    'odoogit.repo_base_path': '/var/lib/odoo/git/repos',
    'odoogit.ssh_host': 'git.example.com',
}
...
def _post_init_hook(env):
    ...
    This hook runs on every `-u odoogit` upgrade too. It used to call
    ...
    _logger.info("OdooGit installed; configuration parameters verified")


def _backfill_git_user_group(env):
    """Put existing employees into group_git_user.

    Every ir.rule in this module is scoped to odoogit.group_git_user, and a
    ...
    """
    git_user = env.ref('odoogit.group_git_user', raise_if_not_found=False)
    ...
    if missing:
        ...
        _logger.info("OdooGit: added %s existing user(s) to Git User",
                     len(missing))
...
def _uninstall_hook(env):
    ...
    _logger.info("OdooGit uninstalled")
